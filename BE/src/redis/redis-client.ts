...
const redisClient: RedisClientType = createClient({ url: REDIS_URI });
...
async function updateScoreInRedis(quizId: string, userId: string, score: number): Promise<void> {
    await redisClient.zIncrBy(`leaderboard:${quizId}`, score, userId);
}

async function getLeaderboardFromRedis(quizId: string): Promise<{ value: string; score: number }[]> {
